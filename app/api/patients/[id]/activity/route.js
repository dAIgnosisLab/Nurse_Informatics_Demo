...
import { activityLogSchema } from '@/lib/validation/clinical'

export async function GET(request, { params }) {
  const { id } = await params
  const data = await prisma.activityLog.findMany({ where: { patientId: id }, orderBy: { recordedAt: 'desc' } })
...
export async function POST(request, { params }) {
  ...
  const result = activityLogSchema.safeParse(body)
